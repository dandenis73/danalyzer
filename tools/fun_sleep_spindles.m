...
%   subsequently reimplemented by Dimitrios Mylonas. Additional edits by
%   Dan Denis
%
...
switch method
    case 'median'
        signalmean = median(coef, 'omitnan'); 
    case 'mean'
        signalmean = mean(coef, 'omitnan'); 
end
...
for chan_i=1:nChannels % Loop for each signal
    ...
    if spindlesAll(chan_i).count == 0 || isnan(spindlesAll(chan_i).count) % no spindles on this channel
        ...
        edgeIdx = find(spindlesAll(chan_i).peakLoc <= srate*2 | spindlesAll(chan_i).peakLoc >= size(coef, 1) - srate*2);

        spindlesAll(chan_i).detSample(edgeIdx)      = [];
        spindlesAll(chan_i).startSample(edgeIdx)    = [];
        spindlesAll(chan_i).endSample(edgeIdx)      = [];
        spindlesAll(chan_i).peakAmp(edgeIdx)        = [];
        spindlesAll(chan_i).peakLoc(edgeIdx)        = [];
        spindlesAll(chan_i).peakFreq(edgeIdx)       = [];
        spindlesAll(chan_i).sigmaPower(edgeIdx)     = [];
        spindlesAll(chan_i).energy(edgeIdx)         = [];
        spindlesAll(chan_i).duration(edgeIdx)       = [];
        spindlesAll(chan_i).numCycles(edgeIdx)      = [];
        spindlesAll(chan_i).numBumps(edgeIdx)       = [];
        spindlesAll(chan_i).symmetry(edgeIdx)       = [];
        spindlesAll(chan_i).freqGradient(edgeIdx)   = [];
        spindlesAll(chan_i).fano(edgeIdx)           = [];
        spindlesAll(chan_i).raisingSlope(edgeIdx)   = [];
        spindlesAll(chan_i).droppingSlope(edgeIdx)  = [];
