...
% events).
%
% 'ArtifactCriteria' = Discard candidate slow oscillations exceeding a
% peak-to-peak amplitude threshold as artifactual. Default = [] (retain all
% events)
%
%
% Outputs:
...
%
% Remarks:
%   Approach based on Staresina et al (2015). Nature Neuroscience. This
%   implementation created by Dimitrios Mylonas. Additional edits made by
%   Dan Denis
%% Default settings
durCrit = [0.5 2];
ampCrit = [];
...
if find(strcmpi(varargin, 'ArtifactCriteria'))
    artCrit = varargin{find(strcmpi(varargin, 'ArtifactCriteria'))+1};
end
...
for chan_i = 1:nChannels
    ...
    if ~isempty(upIdx) && ~isempty(downIdx)
        ...
        if ~isempty(event)
            ...
            if event(end) > length(upIdx)

                lengthDiff = event(end) - length(upIdx);
                event(end-(lengthDiff-1):end) = [];
            end
            ...
            startIdx = downIdx(event);
            endIdx = downIdx(event + 1);

            %% Remove all SOs with peak-to-peak amp > artifact criteria

            if ~isempty(artCrit)
                outlier_so = pp > artCrit;
                startIdx(outlier_so) = [];
                endIdx(outlier_so) = [];
                minIdx(outlier_so)  = [];
                maxIdx(outlier_so)  = [];
            ...
            soAll(chan_i).count       = length(startIdx);            % Number of SO
            soAll(chan_i).minSleep    = size(data,2)/(srate*60);           % Length of sleep
            soAll(chan_i).startSample = startIdx;                    % Start of SO (sample)
            soAll(chan_i).endSample   = endIdx;                % End of SO (sample)
    ...
    disp([' Found ' num2str(soAll(chan_i).count) ' slow oscillations in ' num2str(detectTime) ' seconds'])
    pause(0.1);
