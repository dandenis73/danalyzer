...
%
% Remarks:
%   This implementation created by Dimitrios Mylonas. Additional edits made by
%   Dan Denis
...
for chan_i = 1:nChannels
    ...
    elseif ssEvents(chan_i).count == 0
        ...
        cpAll(chan_i).label        = chans{chan_i};
        cpAll(chan_i).spindleCount = ssEvents(chan_i).count;
        cpAll(chan_i).soCount      = soEvents(chan_i).count;
        cpAll(chan_i).soID         = [];
